...
The add-on also tries to update existing Lovelace resources through the Home Assistant API so old cache suffixes are replaced instead of duplicated. If Home Assistant refuses that API call, add the Resources manually as shown below.
